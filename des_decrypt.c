...
void des_decrypt(char* ciphertext, char* writeFileName, char* key, char* iv, unsigned int cipherlen)
{
    ...
    if (iv == NULL) {
		printf("IV was not provided");
		exit(1);
    }


    int numBlocks = cipherlen / 8 + ((cipherlen % 8) ? 1 : 0);

    unsigned char plaintext[8]; // to place the ciphertext
    unsigned char prev_ciphterText[8];
    ...
    while (i < numBlocks) {
		memcpy(data_block, ciphertext + (8*i), 8);
		if (i == 0) {
			des_decrypt_chunk(key, data_block, plaintext);
			xOrTwoByteArrays(plaintext, iv, 8, 8);
			memcpy(tmp, plaintext, 8);
			tmp[8] = '\0';
			printf("%s", tmp);
		} else {
			des_decrypt_chunk(key, data_block, plaintext);
			xOrTwoByteArrays(plaintext, prev_ciphterText, 8, 8);
			memcpy(tmp, plaintext, 8);
			tmp[8] = '\0';
			printf("%s", tmp);
		}
		memcpy(prev_ciphterText, data_block, 8);
		i++;
    }
	
	printf("\n\nTHANKS FOR USING OUR PROGRAM! COME BACK AGAIN!\n");
}

/**
 * @brief Decryptes 8 bytes at a t ime
...
void des_decrypt_chunk(unsigned char key [], unsigned char C [], unsigned char ret []) {
    ...
    //quick and dirty hack to make our version compatible with openssl
    int i, j, offset = 0;
    ...
    initialPermutation(C, InitPermutation); //for return val
    ...
    for(i = 0; i < 16; i++) {
        if(i ==0) {
            ...
            expansionPermutation(R0, K[15-i], expanded_data);     //Expand the data to 48bits
            xOrTwoByteArrays(L0, expanded_data, 4, 4);
            memcpy(R[i+1], L0, 4);  //R1 = R0
            //print_array_binary(4, R[i+1]);
        } else {
            memcpy(L[i+1], R[i], 4);
            expansionPermutation(R[i], K[15-i], expanded_data);
    ...
    for(i = 0; i < 64; i++) {
        ...
        if(isNthBitSet(revLR[val/8], val % 8)) {
            decrypted_text[i/8] |= (1 << 7-i % 8);
        } else {
            decrypted_text[i/8] &= ~(1 << 7-i % 8);
        }
    }
    
    memcpy(ret, decrypted_text, 8);
}
